#!/usr/bin/env python3
"""
run_task_reach_6dof.py
--------------------

ROS 2 node that drives a Kinova Gen3 6DOF arm with a simple
...
from robots.gen3_6dof import Gen3_6DOFReachPolicy


class ReachPolicy6DOF(Node):
    """ROS2 node for controlling a Gen3 6DOF robot's reach policy."""
    ...
    # Only first 6 joints for 6DOF version
    JOINT_NAMES = [
        'joint_1',
        'joint_2',
        'joint_3',
    ...
    def __init__(self, fail_quietly: bool = False, verbose: bool = False):
        """Initialize the ReachPolicy6DOF node."""
        super().__init__('reach_policy_6dof_node')

        self.robot = Gen3_6DOFReachPolicy()
        self.target_command = np.zeros(7)  # 7D: x, y, z, qx, qy, qz, qw
        ...
        self.get_logger().info("ReachPolicy6DOF node initialized.")
    ...
    def sub_callback(self, msg: JointTrajectoryControllerState):
        ...
        # Update the robot's state with current joint positions and velocities (first 6 joints only)
        positions_6dof = msg.actual.positions[:6]
        velocities_6dof = msg.actual.velocities[:6]
        self.robot.update_joint_state(positions_6dof, velocities_6dof)
    ...
    def step_callback(self):
        ...
        """
        # Set a constant target command for the robot (7D: x, y, z, qx, qy, qz, qw)
        if self.i%3000 < 1000:
